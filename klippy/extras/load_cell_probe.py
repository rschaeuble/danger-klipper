...
class LoadCellProbe:
    ...
    def _find_fit_start(self, gcmd, force0):
        ...
        while abs(force1) > self._fit_threshold * 2:
            ...
            dist = min(
                abs((force2 - self._fit_threshold) * slope), self._step_size / 2
            )
            force1 = force2
    ...
    def cmd_LCP_COMPENSATE(self, gcmd):
        ...
        gcmd.respond_info(
            "Determine zero level from %d samples:" % sample_count
        )
    ...
    def cmd_LCP_CALIB_NOISE(self, gcmd):
        ...
        gcmd.respond_info(
            "Determine noise level from %d samples:" % sample_count
        )
    ...
    def cmd_LCP_CALIB_WEIGHT(self, gcmd):
        ...
        if abs(force) < FIT_THRESHOLD_FACTOR * self._noise_level:
            raise gcmd.error(
                "Measured force too close to noise level: %.6f" % force
            )
    ...
    def LCP_INFO(self, gcmd):
        ...
        gcmd.respond_info(
            "compensation_z_lift = %f" % self._compensation_z_lift
        )
